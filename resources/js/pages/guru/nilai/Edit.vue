...
const form = useForm<Form>({
    ...
    evaluasi_id: props.nilai.evaluasi_id,
    nilai: props.nilai.nilai,
    status: props.nilai.status,
    feedback: props.nilai.feedback,
    tanggal_dinilai: props.nilai.tanggal_dinilai,
});
...
// Initialize dropdown labels with existing values
const selectedStatusLabel = ref(statusOptions.find((s) => s.value === props.nilai.status)?.label || 'Pilih Status');

const selectedSiswaLabel = ref(props.siswaOptions.find((s) => s.id === props.nilai.siswa_id)?.nama || props.nilai.siswa?.nama || 'Pilih Siswa');

const selectedEvaluasiLabel = ref(
    props.evaluasiOptions.find((e) => e.id === props.nilai.evaluasi_id)
        ? `${props.evaluasiOptions.find((e) => e.id === props.nilai.evaluasi_id)?.judul} (${props.evaluasiOptions.find((e) => e.id === props.nilai.evaluasi_id)?.jenis})`
        : props.nilai.evaluasiPembelajaran
          ? `${props.nilai.evaluasiPembelajaran.judul} (${props.nilai.evaluasiPembelajaran.jenis})`
          : 'Pilih Evaluasi',
);

const submit = () => {
    form.put(route('guru.nilai.update', props.nilai.id), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    ...
    <AppLayout
        :breadcrumbs="[
            ...
            { title: 'Edit Nilai', href: route('guru.nilai.edit', props.nilai.id) },
        ]"
    >
        <div class="flex flex-col gap-4 px-10 py-6">
            ...
            <form class="grid w-full max-w-full grid-cols-1 gap-4 md:max-w-lg md:grid-cols-2" @submit.prevent="submit">
                ...
                <div class="mt-4 flex gap-2 md:col-span-2">
                    <Button variant="outline" type="button" @click="router.visit(route('guru.nilai.index'))">Batal</Button>
